<?php
define('TITLE', 'View Students');
define('PAGE', 'viewstudent');
...
if (isset($_POST['stsnoedit'])) {
  ...
  $sql = "UPDATE `student_tb` SET `student_name` = '$sname', `student_roll` = '$sroll', `student_gender` = '$sgender', `student_parent` = '$sparent', `student_mobile` = '$smobile' WHERE `student_tb`.`student_id` = $sno";
  ...
  <table class="mx-2 table table-striped" id="studentTable">
    ...
    <tbody>
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        <td> <button class='stedit btn btn-sm btn-info' id=" . $row['student_id'] . "><i class='fas fa-pen'></i></button> <button class='stdelete btn btn-sm btn-danger' id=" . $row['student_id'] . "><i class='far fa-trash-alt'></i></button> </td>
      </tr>";
      }
      ?>
    </tbody>
